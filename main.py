...
from utils.bn_type import STRBatchNorm
from utils.bn_type import sparseFunction
...
def main_worker(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # Storing sparsity and threshold statistics for STRBN models
        if args.bn_type == "STRBatchNorm":
            count = 0
            sum_sparse = 0.0
            for n, m in model.named_modules():
                if isinstance(m, STRBatchNorm):
                    sparsity, total_params, thresh = m.getSparsity()
                    writer.add_scalar("structured_sparsity/{}".format(n), sparsity, epoch)
            ...
            writer.add_scalar("structured_sparsity/total", total_sparsity, epoch)
        writer.add_scalar("test/lr", cur_lr, epoch)
        end_epoch = time.time()
    ...
    if args.bn_type == "STRBatchNorm":
        ...
        if not os.path.exists("runs/layerwise_structured_sparsity"):
            os.mkdir("runs/layerwise_structured_sparsity")
        if not os.path.exists("runs/layerwise_threshold"):
            os.mkdir("runs/layerwise_structured_threshold")
        with open("runs/layerwise_structured_sparsity/{}.json".format(args.name), "w") as f:
            json.dump(json_data, f)
        with open("runs/layerwise_structured_threshold/{}.json".format(args.name), "w") as f:
